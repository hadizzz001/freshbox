...
export async function PATCH(request, { params }) {
    ...
    try {
        ...
        const currentStock = parseInt(product.stock1, 10);
        ...
        const updatedProduct = await collection.findOneAndUpdate(
            { _id: new ObjectId(id) },
            { $set: { stock1: (currentStock - quantityToDecrease).toString() } }, // Store as a string again if needed
...
export async function GET(request, { params }) {
    ...
    try {
      ...
      const product = await collection.findOne(
        { _id: new ObjectId(id) }, 
        { projection: { stock1: 1 } } // Only fetch the stock field
      );
      ...
      const stock = parseInt(product.stock1, 10);
